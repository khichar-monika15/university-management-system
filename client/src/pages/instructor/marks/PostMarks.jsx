...
export default function PostMarks() {
  ...
  async function handleSubmit(event) {
    ...
    if (!marksData.length) { toast.error("Load students first.", toastErrorObject); return; }
      ...
      <form onSubmit={handleSubmit}>
        <div className="row mb-3">
          ...
          <div className="col-md-2">
            ...
            <input type="number" className="form-control" value={totalMarks} onChange={(e) => setTotalMarks(e.target.value)} min="1" required />
          </div>
          <div className="col-md-2 d-flex align-items-end gap-2">
            <button type="button" className="btn btn-secondary" onClick={handleLoad}>Load</button>
            <button type="submit" className="btn btn-dark" disabled={isSubmitting}>{isSubmitting ? "Saving..." : "Submit"}</button>
          </div>
        </div>
      </form>
      <MarksTable
